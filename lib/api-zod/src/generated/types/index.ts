/**
 * Generated by orval v8.5.3 🍺
 * Do not edit manually.
 * Api
 * CryptoX Exchange API
 * OpenAPI spec version: 0.1.0
 */

export * from "./adminCreateRequest";
export * from "./adminCustomEmailInput";
export * from "./adminCustomEmailInputAudience";
export * from "./adminCustomEmailResult";
export * from "./adminCustomEmailResultStatus";
export * from "./adminKycIdNumber";
export * from "./adminKycSubmission";
export * from "./adminLoan";
export * from "./adminLoanIdNumber";
export * from "./adminLoanIdType";
export * from "./adminStats";
export * from "./adminTransaction";
export * from "./adminUserPreview";
export * from "./adminUserPreviewHolding";
export * from "./adminUserPreviewTransaction";
export * from "./adminUserUpdate";
export * from "./adminUserUpdateKycStatus";
export * from "./adminUserUpdateRole";
export * from "./adminUserUpdateStatus";
export * from "./authResponse";
export * from "./changePasswordRequest";
export * from "./coinPrice";
export * from "./convertRequest";
export * from "./convertResponse";
export * from "./createInvestmentRequest";
export * from "./depositRequest";
export * from "./errorResponse";
export * from "./getAdminTransactionsParams";
export * from "./getAdminUsersParams";
export * from "./healthStatus";
export * from "./holding";
export * from "./investment";
export * from "./investmentStatus";
export * from "./kycSubmitRequest";
export * from "./loan";
export * from "./loanApplicationInput";
export * from "./loanApplicationInputIdType";
export * from "./loanRejectionInput";
export * from "./loanStatus";
export * from "./loginRequest";
export * from "./messageResponse";
export * from "./portfolio";
export * from "./referralSummary";
export * from "./registerRequest";
export * from "./tradeRequest";
export * from "./transaction";
export * from "./transactionStatus";
export * from "./transactionType";
export * from "./user";
export * from "./userKycStatus";
export * from "./userRole";
export * from "./userStatus";
export * from "./withdrawalAddress";
export * from "./withdrawRequest";
export * from "./withdrawRequestMethod";
