...
- Spendable balance = TOTAL account value across all coins, not just USDT: investments and USDT-side buys debit via `debitUsdAcrossHoldings` (api-server `lib/balance.ts`) — USDT first, then other coins at live prices, per-leg conditional UPDATEs, always inside one transaction with the credit + ledger insert. Convert/invest must never commit a debit before the matching credit.
